...
// Temporary module declaration for '@uploadcare/file-uploader' to fix import error
// Remove this when proper types are installed
// @ts-ignore
...
export default function FileUploader({ files, uploaderClassName, onChange, theme }: FileUploaderProps) {
  ...
  // Update handler to match OutputCollectionState signature
  const handleChangeEvent = (event: any) => {
    // event.successEntries contains successfully uploaded files
    setUploadedFiles(event.successEntries as OutputFileEntry<'success'>[]);
  }
